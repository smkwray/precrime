...
def write_stability_report(best_models_path: Path | None = None, out_path: Path | None = None, seeds: list[int] | None = None) -> Path:
    ...
    lines: list[str] = [
        ...
        "",
        "Policy for error-rate summary: `top10%` (flag top 10% highest predicted risks; threshold derived from the held-out selection split).",
        "",
    ]
    ...
    for horizon in sorted(best.keys()):
        ...
        for seed in seeds:
            # 4-way split: thresholds from select split
            fit_idx, cal_idx, select_idx, test_idx = split_train_cal_select_test(y_full, seed=seed)
            ...
            # Race gap at top10% — threshold derived from select split
            ...
            if "Race" in ds.columns:
                ...
                thr = _threshold_top_k(p_select, 0.10)
                fprs = []
                fnrs = []
        ...
        for key, label in [
            ...
            m, s = _mean_std(key)
            lines.append(f"| {label} | {format_float(m)} | {format_float(s)} |")
        lines.append("")
